...
Commit `bfc636d` deployed to both Laravel apps (autoDeploy, confirmed via `git -C /app log -1` on each container).

**Aldemar**: 0 candidates (`polling_place_source` non-null AND `polling_place_id` null) — not affected.

**sigma-betha** (`census:backfill-polling-place-id`, dry-run then real): 526 candidates, all `live`-sourced. Real run: **512 de 526 apoyo(s) fueron enlazados a su puesto de votación. 14 no pudieron re-derivarse con datos locales** — the 14 unmatched are voters whose `registraduria_lookups` row's `municipio` field doesn't match any row in `municipalities` (expected, safe skip per the resolver's existing `resolveOrCreatePollingPlace()` null-guard, not a bug). No live/2captcha adapter calls were made (command only reads `registraduria_lookups`/`national_census_records`/`census_records` locally).
