...
- [x] **SRC-01**: Every polling-place result visibly shows whether it came from a live source, database reconstruction, local snapshot, or manual entry
...
- [x] **SRC-04**: Operator can manually trigger a re-check of a voter's polling place at any time
- [x] **SRC-05**: Operator can filter/view which voters currently have a fallback-sourced (non-live) polling place
...
- [x] **LIVE-02**: Feasibility of `wsp.registraduria.gov.co` (reCAPTCHA Enterprise) as an additional live-source adapter is validated end-to-end before the system relies on it
...
| SRC-01 | Phase 10 | Done |
| SRC-02 | Phase 8 | Complete |
| SRC-03 | Phase 7 | Done |
| SRC-04 | Phase 10 | Done |
| SRC-05 | Phase 10 | Done |
