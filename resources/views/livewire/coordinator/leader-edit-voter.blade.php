...
layout('components.layouts::app', ['title' => 'Editar Información Adicional']);
...
<div class="mx-auto max-w-2xl space-y-6 p-6">
    <div class="flex items-center gap-4">
        <flux:button variant="ghost" :href="route('coordinator.leaders.voters', $leader)" wire:navigate icon="arrow-left" size="sm">
            Volver
        </flux:button>
        <div class="flex-1">
            <flux:heading size="xl">Editar Información Adicional</flux:heading>
            <flux:subheading>Apoyo de {{ $leader->name }} — {{ $voter->first_name }} {{ $voter->last_name }} ({{ $voter->document_number }})</flux:subheading>
        </div>
    </div>
    ...
    <form wire:submit="save" class="flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-3 sm:flex-row">
            ...
            <flux:button variant="ghost" :href="route('coordinator.leaders.voters', $leader)" wire:navigate class="flex-1">
                Cancelar
            </flux:button>
        </div>
    </form>
